...
fn bitcast_internal<I, O>(input_bytes: &[u8], output_buf: &mut Vec<u8>) -> Result<(), PhoenixError>
where
    I: Pod,
    O: Pod,
{
    ...
    let input_slice: &[I] = bytemuck::try_cast_slice(input_bytes).map_err(|e| {
        PhoenixError::InternalError(format!("Failed to cast bytes to input type: {}", e))
    })?;
...
pub fn encode<I, O>(input_bytes: &[u8], output_buf: &mut Vec<u8>) -> Result<(), PhoenixError>
where
    I: Pod,
    O: Pod,
{
...
pub fn decode<I, O>(input_bytes: &[u8], output_buf: &mut Vec<u8>) -> Result<(), PhoenixError>
where
    I: Pod,
    O: Pod,
{
...
mod tests {
    ...
    fn test_bitcast_size_mismatch_error() {
        ...
        if let Err(PhoenixError::InternalError(msg)) = result {
