...
use crate::utils; // Assuming utils has our safe casting function
...
pub fn plan(py: Python, bytes: &[u8], original_type: &str) -> PyResult<String> {
    ...
    py.allow_threads(|| {
        ...
        let result = match original_type {
            ...
            _ => {
                ...
                Ok(serde_json::to_string(&pipeline).unwrap()) // This unwrap needs to be fixed
            }
        };

        // Convert our PhoenixError to a PyErr before returning to Python
