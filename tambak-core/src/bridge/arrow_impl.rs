...
/// * `Err(tambakError)` if the type is unsupported.
///
...
pub fn arrow_to_pipeline_input(array: &dyn Array) -> Result<PipelineInput, tambakError> {
    ...
    let main_data = {
        ...
        match array.data_type() {
            ...
            dt => {
                return Err(tambakError::UnsupportedType(format!(
                    "Unsupported type: {}",
                    dt
                )))
            }
    ...
    Ok(PipelineInput::new(
        ...
        TambakDataType::from_arrow_type(array.data_type())?,
        array.len(),
...
pub fn pipeline_output_to_array(output: PipelineOutput) -> Result<Box<dyn Array>, tambakError> {
    ...
    match output.initial_dtype {
        TambakDataType::Int8 => reapply_and_build!(Int8Type),
        TambakDataType::Int16 => reapply_and_build!(Int16Type),
        TambakDataType::Int32 => reapply_and_build!(Int32Type),
        TambakDataType::Int64 => reapply_and_build!(Int64Type),
        TambakDataType::UInt8 => reapply_and_build!(UInt8Type),
        TambakDataType::UInt16 => reapply_and_build!(UInt16Type),
        TambakDataType::UInt32 => reapply_and_build!(UInt32Type),
        TambakDataType::UInt64 => reapply_and_build!(UInt64Type),
        TambakDataType::Float32 => reapply_and_build!(Float32Type),
        TambakDataType::Float64 => reapply_and_build!(Float64Type),
