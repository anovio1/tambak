...
/// This allows the `tambakBatchReader` to be agnostic of the underlying file structure.
...
/// A high-level, stateful object that manages the decompression of a tambak file.
...
impl<R: Read + Seek + Send> Decompressor<R> {
    pub fn new(mut source: R) -> Result<Self, tambakError> {
        source.seek(SeekFrom::Start(0))?;
        let mut magic_buf = [0u8; 4];
        source
            .read_exact(&mut magic_buf)
            .map_err(|e| tambakError::Io(e))?;
        ...
        if footer_len == 0 {
            return Err(tambakError::FrameFormatError(
                "Footer length is zero, invalid tambak file.".into(),
            ));
        }
    ...
    pub fn batched<'a>(self) -> Result<tambakBatchReader<'a, R>, tambakError>
    where
        Self: 'a,
    {
        let mode: Box<dyn DecompressionMode<R>> = if let Some(frame_plan) = self.frame_plan.as_ref()
        {
            ...
            {
                return Err(tambakError::FrameFormatError("This is a partitioned file. Please use the `.partitions()` method instead of `.batched()`.".to_string()));
            }
    ...
    pub fn partitions(self) -> Result<PartitionIterator<R>, tambakError> {
...
/// An iterator that yields `RecordBatch`es from a non-partitioned tambak file stream.
pub struct tambakBatchReader<'a, R: Read + Seek + Send + 'a> {
    mode: Box<dyn DecompressionMode<R> + 'a>,
}

impl<'a, R: Read + Seek + Send + 'a> Iterator for tambakBatchReader<'a, R> {
...
impl<'a, R: Read + Seek + Send + 'a> RecordBatchReader for tambakBatchReader<'a, R> {
    fn schema(&self) -> SchemaRef {
        self.mode.schema()
    }
}
