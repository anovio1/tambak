...
//! throughout the tambak Rust core.
//!
...
///
/// # Errors
/// Returns a `tambakError::BufferMismatch` if the byte slice length is not
/// perfectly divisible by the size of the target type `T`.
//  CURRENT V3
pub fn safe_bytes_to_typed_slice<'a, T>(bytes: &'a [u8]) -> Result<&'a [T], tambakError>
where
    T: bytemuck::Pod, // Use bytemuck's trait for "Plain Old Data"
{
    bytemuck::try_cast_slice(bytes).map_err(|e| {
        ...
        tambakError::InternalError(format!("Failed to cast byte slice: {}", e))
    })
}

//  BEFORE V2
// pub fn safe_bytes_to_typed_slice<'a, T: PrimInt>(bytes: &'a [u8]) -> Result<&'a [T], tambakError> {
//     let type_size = std::mem::size_of::<T>();
//     if type_size == 0 { // Should not happen for PrimInt, but good practice
//         return Ok(&[]);
//     }
//     if bytes.len() % type_size != 0 {
//         return Err(tambakError::BufferMismatch(bytes.len(), type_size));
//     }
...
//     if bytes.len() % size != 0 {
//         return Err(tambakError::BufferMismatch(bytes.len(), size).into());
//     }
//     let ptr = bytes.as_ptr() as *const T;
...
///
/// # Errors
/// Returns a `tambakError::UnsupportedType` if the string is not recognized.
pub fn get_element_size(type_str: &str) -> Result<usize, tambakError> {
    match type_str {
        "Int8" | "UInt8" | "Boolean" => Ok(1),
        "Int16" | "UInt16" => Ok(2),
        "Int32" | "UInt32" => Ok(4),
        "Int64" | "UInt64" => Ok(8),
        _ => Err(tambakError::UnsupportedType(type_str.to_string())),
    }
}
...
mod tests {
    ...
    fn test_safe_bytes_to_typed_slice_mismatch_error() {
        ...
        assert!(matches!(result_i32, Err(tambakError::InternalError(_))));

        // Test with i16
        let result_i16 = safe_bytes_to_typed_slice::<i16>(&bytes);
        assert!(result_i16.is_err());
        assert!(matches!(result_i16, Err(tambakError::InternalError(_))));
    }
    ...
    fn test_get_element_size_unsupported() {
        ...
        if let Err(tambakError::UnsupportedType(s)) = result {
            assert_eq!(s, "Float32");
        } else {
