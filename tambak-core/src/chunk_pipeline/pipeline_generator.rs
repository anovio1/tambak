...
use crate::types::tambakDataType; // Using the shared type from the library
...
impl<'a> PipelineGenerator<'a> {
    fn new(
        ...
        let initial_physical_type = match logical_type {
            LogicalType::SignedInteger => tambakDataType::Int64, // Assume widest for planning
            LogicalType::UnsignedInteger => tambakDataType::UInt64,
            LogicalType::Float => tambakDataType::Float64,
            LogicalType::Other => tambakDataType::UInt8, // Treat as bytes
        };
    ...
    /// Returns `Ok(())` for success, `Err(tambakError)` if a branch is invalid.
    fn generate_next_steps(&mut self, item: WorkItem) -> Result<(), tambakError> {
        ...
        if logical_type == LogicalType::Float && physical_type.is_float() {
            let bitcast_to = if physical_type == tambakDataType::Float32 {
        ...
        if logical_type == LogicalType::SignedInteger {
            ...
            self.add_to_queue(delta_item, vec![Operation::ZigZag], tambakDataType::UInt64);
    ...
    fn generate_sparsify_pipelines(&mut self, item: &WorkItem) -> Result<(), tambakError> {
    ...
    fn add_to_queue(
        ...
        mut item: WorkItem,
        ops: Vec<Operation>,
        expected_next_physical_type: tambakDataType,
    ) -> Result<(), tambakError> {
        ...
        if temp_type != expected_next_physical_type {
            return Err(tambakError::StreamTransformError(format!(
