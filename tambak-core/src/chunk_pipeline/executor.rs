...
//! This module contains the v4.3 core execution logic for the tambak compression
...
pub(crate) fn execute_linear_decode_pipeline(
    bytes: &[u8],
    type_flow: &mut Vec<TambakDataType>,
    pipeline: &[Operation],
    num_values: usize,
) -> Result<Vec<u8>, tambakError> {
    if pipeline.is_empty() {
    ...
    let mut current_type = type_flow.pop().ok_or_else(|| {
        tambakError::InternalError("Type flow stack is empty at start of decode.".to_string())
    })?;

    for op in pipeline.iter().rev() {
        let target_type = type_flow.pop().ok_or_else(|| {
            tambakError::InternalError(format!(
