...
fn shuffle_slice<T>(input_slice: &[T], output_buf: &mut Vec<u8>) -> Result<(), tambakError>
where
...
fn unshuffle_slice<T>(input_bytes: &[u8], output_buf: &mut Vec<u8>) -> Result<(), tambakError>
where
    ...
    T: bytemuck::Pod,
{
    ...
    if input_bytes.len() % element_size != 0 {
        return Err(tambakError::BufferMismatch(input_bytes.len(), element_size));
    }
...
mod tests {
    ...
    fn test_decode_invalid_length_error() {
        ...
        if let Err(e) = result {
            assert!(matches!(e, tambakError::BufferMismatch(7, 2)));
        }
    }
