...
pub fn split_stream<T>(data: &[T]) -> Result<(Vec<bool>, Vec<T>), tambakError>
where
    T: Pod + Zero,
{
    ...
    T: Pod + Zero,
{
    if mask.len() != total_len {
        return Err(tambakError::SparsityError(format!(
    ...
    for &is_valid in mask {
        if is_valid {
            if let Some(&value) = dense_iter.next() {
                ...
                return Err(tambakError::SparsityError(
...
mod tests {
    ...
    fn test_reconstruct_errors() {
        ...
        assert!(matches!(result, Err(tambakError::SparsityError(_))));
