...
//! and `tambakDataType` and calls the appropriate generic kernel implementation,
...
pub fn dispatch_encode(
    ...
    macro_rules! convert_and_exec_encode {
        ($T:ty, $kernel:ident, $($args:expr),*) => {{
            if input_bytes.len() % std::mem::size_of::<$T>() != 0 {
                return Err(tambakError::BufferMismatch(input_bytes.len(), std::mem::size_of::<$T>()));
            }
    ...
    match op {
        ...
        Sparsify { .. } | ExtractNulls { .. } => Err(tambakError::InternalError(
...
pub fn dispatch_decode(
    ...
    input_bytes: &[u8],
    output_buf: &mut Vec<u8>,
    current_dtype: TambakDataType, // The type of the input_bytes
    target_dtype: TambakDataType,  // The type we expect to GET BACK
    num_values: usize,
) -> Result<(), tambakError> {
...
pub fn dispatch_split_stream(
    input_bytes: &[u8],
    dtype: TambakDataType,
) -> Result<(Vec<bool>, Vec<u8>), tambakError> {
...
mod tests {
    ...
    fn test_dispatch_zigzag_roundtrip_logic() {
        ...
        let original_type = TambakDataType::Int32;
    ...
    fn test_dispatch_bitpack_roundtrip() {
        ...
        let original_type = TambakDataType::UInt32;
    ...
    fn test_dispatch_unsupported_type_error() {
        ...
        if let Err(tambakError::UnsupportedType(msg)) = result {
