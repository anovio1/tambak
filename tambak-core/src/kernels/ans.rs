...
fn build_normalized_symbols(
    input_bytes: &[u8],
) -> Result<([AnsSymbol; 256], Vec<(u8, u16)>), tambakError> {
    if input_bytes.is_empty() {
        return Err(tambakError::AnsError(
    ...
    if final_cum_freq != ANS_NORMALIZATION_FACTOR {
        return Err(tambakError::AnsError(format!(
...
///
/// # Returns
/// A `Result` containing the compressed `Vec<u8>` or an `tambakError`.
pub fn encode(input_bytes: &[u8]) -> Result<Vec<u8>, tambakError> {
    if input_bytes.is_empty() {
        return Ok(Vec::new());
    }
    ...
    let uncompressed_len: u64 = input_bytes.len().try_into().map_err(|_| {
        tambakError::AnsError("Input data length exceeds u64 capacity.".to_string())
    })?;
...
///
/// # Returns
/// A `Result` containing the decompressed `Vec<u8>` or an `tambakError`.
pub fn decode(input_bytes: &[u8]) -> Result<Vec<u8>, tambakError> {
    ...
    let mut read_bytes = |len: usize| -> Result<&[u8], tambakError> {
        let end = cursor + len;
        let slice = input_bytes.get(cursor..end).ok_or_else(|| {
            tambakError::AnsError("Input stream is truncated or corrupt.".to_string())
        })?;
        cursor = end;
        Ok(slice)
    };
...
pub fn encode_fast(input_bytes: &[u8]) -> Result<Vec<u8>, tambakError> {
    if input_bytes.is_empty() {
        return Ok(Vec::new());
    }
...
pub fn decode_fast(input_bytes: &[u8]) -> Result<Vec<u8>, tambakError> {
    if input_bytes.is_empty() {
        return Ok(Vec::new());
    }
...
pub fn encode_fast_no_interleave(input_bytes: &[u8]) -> Result<Vec<u8>, tambakError> {
    if input_bytes.is_empty() {
        return Ok(Vec::new());
    }
...
pub fn decode_fast_no_interleave(input_bytes: &[u8]) -> Result<Vec<u8>, tambakError> {
    if input_bytes.is_empty() {
        return Ok(Vec::new());
    }
...
mod tests {
    ...
    fn test_ans_empty_and_single_symbol() {
        ...
        assert!(
            matches!(result, Err(tambakError::AnsError(msg)) if msg.contains("at least two unique symbols"))
        );
    }
    ...
    fn test_decode_corrupt_data_truncated() {
        ...
        for i in 1..encoded.len() {
            ...
            assert!(
                matches!(&result, Err(tambakError::AnsError(msg)) if msg.contains("truncated or corrupt")),
                "Failed on length {}",
                i
            );
        }
    ...
    fn test_decode_invalid_frequency_sum() {
        ...
        assert!(
            matches!(&result, Err(tambakError::AnsError(msg)) if msg.contains("Corrupt header: frequencies sum to"))
        );
    }
}
