...
fn encode_slice<T>(data: &[T], bit_width: u8) -> Result<BitVec<u8, Lsb0>, tambakError>
where
    T: PrimInt + Unsigned + ToPrimitive,
{
    if bit_width == 0 || bit_width > (std::mem::size_of::<T>() * 8) as u8 {
        return Err(tambakError::BitpackEncodeError(0, bit_width));
    }
    ...
    for &val in data {
        let val_u64 = val.to_u64().ok_or_else(|| tambakError::UnsupportedType("Failed to convert value to u64 for bitpacking".to_string()))?;
        if val_u64 > max_val {
            return Err(tambakError::BitpackEncodeError(val_u64, bit_width));
        }
...
fn decode_slice<T>(
    ...
    bit_width: u8,
    num_values: usize,
) -> Result<Vec<T>, tambakError>
where
    T: PrimInt + Unsigned + TryFrom<u64>,
{
    if bit_width == 0 {
        return if num_values == 0 { Ok(Vec::new()) } else { Err(tambakError::BitpackDecodeError) };
    }
    if bits.len() < num_values * bit_width as usize {
        return Err(tambakError::BitpackDecodeError);
    }
...
mod tests {
    ...
    fn test_decode_truncated_buffer_error() {
        ...
        if let Err(e) = result {
            assert!(matches!(e, tambakError::BitpackDecodeError));
        }
    }
    ...
    fn test_encode_value_exceeds_bit_width_error() {
        ...
        if let tambakError::BitpackEncodeError(val, width) = err {
            assert_eq!(val, 8);
            assert_eq!(width, 3);
        } else {
