...
///
/// # Returns
/// An empty `Result` on success, or a `tambakError` on failure.
...
///
/// # Returns
/// An empty `Result` on success, or a `tambakError` if the buffer is corrupt
/// or malformed.
pub fn decode<T: Copy + Pod>( // Keep Copy trait bound for now
    ...
    let dict_len_bytes: [u8; 4] = input_bytes
        ...
        .ok_or_else(|| tambakError::DictionaryError("Truncated header: cannot read dictionary length".to_string()))?;
    ...
    let dict_bytes = input_bytes
        ...
        .ok_or_else(|| tambakError::DictionaryError("Truncated dictionary data".to_string()))?;
    ...
    let indices_bytes = input_bytes
        .get(cursor..)
        .ok_or_else(|| tambakError::DictionaryError("Missing indices data".to_string()))?;
    
    if indices_bytes.len() % u32_size != 0 {
        return Err(tambakError::DictionaryError(
    ...
    if indices.len() != num_values {
        return Err(tambakError::DictionaryError(format!(
    ...
    for &index in &indices {
        let value = dictionary.get(index as usize).ok_or_else(|| {
            tambakError::DictionaryError(format!(
...
mod tests {
    ...
    fn test_decode_errors() {
        ...
        assert!(matches!(err, tambakError::DictionaryError(_)));
