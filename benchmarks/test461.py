...
try:
    ...
    logging.error(f"Failed to import from tubuin-processor. Check TUBUIN_PROCESSOR_PATH. Error: {e}")
    sys.exit(1)
...
def validate_roundtrip(aspect_name: str, strategy_name: str):
        ...
        with open(phx_output_path, "rb") as f:
            ...
            if strategy_name == "partitioned":
                ...
                for _, partition_reader in decompressor.partitions():
                    # CORRECT: Use the .read_all() method to consume the reader
                    partition_table = partition_reader.read_all()
                    all_tables.append(partition_table)
                decompressed_table = pa.concat_tables(all_tables) if all_tables else pa.Table.from_pylist([])
            else: # For 'none' and 'per_batch_relinearize'
                reader = decompressor.batched()
                
                # CORRECT: Use the .read_all() method here as well
                decompressed_table = reader.read_all()
        
        logger.info(f"-> Decompression complete. Reconstructed table with {decompressed_table.num_rows} rows.")
    except Exception as e:
    ...
    if strategy_name == "partitioned":
        ...
        logger.info("-> Sorting tables for comparison (partitioning does not preserve order).")
        sort_keys = original_table.column_names
        original_to_compare = original_table.sort_by([(key, "ascending") for key in sort_keys])
        decompressed_to_compare = decompressed_table.sort_by([(key, "ascending") for key in sort_keys])
    else:
    ...
    if original_to_compare.equals(decompressed_to_compare):
        ...
        logger.error("❌ FAILURE: Roundtrip validation failed. Data differs after decompression.")
